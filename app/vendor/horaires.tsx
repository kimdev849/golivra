...
export default function VendorHorairesScreen() {
  ...
  // ─── Picker state ──────────────────────────────────────────────────────
  // Android: native clock dialog (no Modal). iOS: bottom-sheet Modal + spinner.
  ...
  useEffect(() => {
    let alive = true;
    const load = async () => {
      if (!enterpriseId) { if (alive) setLoading(false); return; }
      try {
  ...
  // ─── iOS: close / apply ────────────────────────────────────────────────
  const closeIosPicker = useCallback(() => { setIosPicker(null); setPendingTime(null); }, []);
  ...
  // The native Android clock dialog fires onChange multiple times during
  // the 2-step selection (hour → minute). We keep the picker open and only
  // apply the final value when the user confirms (type='set') or cancels
  // (type='dismissed').
  const handleAndroidChange = useCallback(
    (event: DateTimePickerEvent, date?: Date) => {
      if (event.type === 'dismissed') {
        // User cancelled — close without applying
        setAndroidPicker(null);
        return;
      }
      if (event.type === 'set' && date && androidPicker) {
        // Apply the selected time and close
        updateDay(androidPicker.jour, { [androidPicker.field]: date });
        setAndroidPicker(null);
      }
    },
    [androidPicker, updateDay],
  );

  const allOpen = () => setDays(defaultDays());
  const allClosed = () =>
    setDays(DAY_NAMES.map((_, jour) => ({
      jour, ouvert: false,
      ouverture: new Date(2000, 0, 1, 9, 0),
      fermeture: new Date(2000, 0, 1, 22, 0),
    })));
  ...
  const previewRows = useMemo<EnterpriseHoraires[]>(
    () => days.filter((d) => d.ouvert).map((d) => ({
      jour: d.jour, ouverture: hourFromDate(d.ouverture), fermeture: hourFromDate(d.fermeture),
    })),
    [days],
  );
  ...
  const previewSoft = previewRows.length === 0 ? colors.errorSoft : preview.open ? colors.successSoft : colors.warningSoft;

  let previewTitle: string;
  let previewSub: string;
  if (previewRows.length === 0) {
    previewTitle = 'Fermé'; previewSub = 'Ouvrez au moins un jour pour recevoir des commandes.';
  } else if (preview.open) {
    previewTitle = "Ouvert aujourd\u2019hui";
    previewSub = preview.todayHours ? `Vos clients peuvent commander \u00b7 ${preview.todayHours}` : 'Vos clients peuvent commander.';
  } else if (preview.nextLabel.startsWith('aujourd')) {
    previewTitle = 'Fermé pour le moment'; previewSub = `Rouvre ${preview.nextLabel}.`;
  } else if (preview.nextLabel) {
    previewTitle = "Fermé aujourd\u2019hui"; previewSub = `Réouverture ${preview.nextLabel}.`;
  } else {
    previewTitle = 'Fermé'; previewSub = 'Aucune ouverture prévue cette semaine.';
  }

  const save = async () => {
    if (!enterpriseId) { setError('Commerce introuvable. Réessayez.'); return; }
    setSaving(true); setError(null);
    try {
      const token = await getSessionToken();
      if (!token) throw new Error('Session expirée.');
      const rows: EnterpriseHoraires[] = days.filter((d) => d.ouvert).map((d) => ({
        jour: d.jour, ouverture: hourFromDate(d.ouverture), fermeture: hourFromDate(d.fermeture),
      }));
  ...
  return (
    <ThemedView style={styles.screen}>
      ...
      <ScrollView showsVerticalScrollIndicator={false} contentContainerStyle={[styles.scroll, { paddingBottom: Math.max(insets.bottom, 12) + 168 }]}>
        ...
        <View style={[styles.infoBanner, { backgroundColor: colors.surfaceMuted, borderColor: colors.border }]}>
          ...
          <ThemedText style={[styles.infoText, { color: colors.textSecondary }]}>
            Vos clients ne peuvent commander que pendant ces horaires. Le changement s'applique immédiatement.
          </ThemedText>
        </View>
        ...
        <ThemedText style={[styles.sectionLabel, { color: colors.textSecondary }]}>Jours de la semaine</ThemedText>
                  ...
                  <View style={styles.dayRow}>
                    <View style={{ flex: 1 }}>
                      <View style={styles.dayNameRow}>
                        <ThemedText style={[styles.dayName, { color: d.ouvert ? colors.text : colors.textMuted }]}>{DAY_NAMES[d.jour]}</ThemedText>
                        {isToday ? (
                          <View style={[styles.todayPill, { backgroundColor: colors.primarySoft }]}>
                            <ThemedText style={[styles.todayPillTxt, { color: colors.primary }]}>Aujourd'hui</ThemedText>
                          </View>
                        ) : null}
                      </View>
                      {d.ouvert ? (
                        <View style={styles.timeRow}>
                          <Pressable style={[styles.timeChip, dayStyle]} onPress={() => openPicker({ jour: d.jour, field: 'ouverture', value: d.ouverture })} accessibilityRole="button">
                            <Clock size={13} color={colors.primary} strokeWidth={LUCIDE_STROKE} />
                            <ThemedText style={[styles.timeChipTxt, { color: colors.primary }]}>{hourFromDate(d.ouverture)}</ThemedText>
                          </Pressable>
                          <ThemedText style={[styles.timeSep, { color: colors.textMuted }]}>—</ThemedText>
                          <Pressable style={[styles.timeChip, dayStyle]} onPress={() => openPicker({ jour: d.jour, field: 'fermeture', value: d.fermeture })} accessibilityRole="button">
                            <Clock size={13} color={colors.primary} strokeWidth={LUCIDE_STROKE} />
                            <ThemedText style={[styles.timeChipTxt, { color: colors.primary }]}>{hourFromDate(d.fermeture)}</ThemedText>
                          </Pressable>
                        </View>
                      ) : (
                    ...
                    <Switch value={d.ouvert} onValueChange={(v) => updateDay(d.jour, { ouvert: v })} trackColor={{ true: colors.primary, false: colors.borderStrong }} thumbColor="#FFFFFF" ios_backgroundColor={colors.borderStrong} />
                  </View>
                </View>
              );
            })}
        ...
        <View style={[styles.summaryRow, { backgroundColor: colors.surfaceMuted, borderColor: colors.border }]}>
          ...
          <ThemedText style={[styles.summaryTxt, { color: colors.textSecondary }]} numberOfLines={2}>{summary}</ThemedText>
        </View>
      </ScrollView>
        ...
        <Pressable style={({ pressed }) => [styles.saveBtn, { backgroundColor: saving || loading ? colors.borderStrong : colors.primary }, pressed && styles.pressedDim]} disabled={saving || loading} onPress={() => void save()}>
          {saving ? <ActivityIndicator color="#FFFFFF" size="small" /> : (
            <>
              ...
              <ThemedText style={styles.saveText}>{loading ? 'Chargement…' : openCount > 0 ? `Enregistrer · ${openCount} jour${openCount > 1 ? 's' : ''}` : 'Enregistrer'}</ThemedText>
            </>
          )}
        </Pressable>
      </View>

      {/* ── Android: native clock dialog (no Modal!) ── */}
      ...
      {!IS_ANDROID && (
        <Modal visible={iosPicker !== null} transparent animationType="fade" onRequestClose={closeIosPicker}>
          ...
          <View style={[styles.sheet, { backgroundColor: colors.surfaceElevated, borderTopColor: colors.border }]}>
            <View style={[styles.sheetHandle, { backgroundColor: colors.borderStrong }]} />
            <ThemedText style={[styles.sheetTitle, { color: colors.text }]}>
              {iosPicker ? `${DAY_NAMES[iosPicker.jour]} · ${iosPicker.field === 'ouverture' ? 'Ouverture' : 'Fermeture'}` : ''}
            </ThemedText>
            {iosPicker ? (
              <DateTimePicker value={pendingTime ?? iosPicker.value} mode="time" is24Hour display="spinner" accentColor={colors.primary} onChange={(_e, date) => { if (date) setPendingTime(date); }} />
            ) : null}
            <View style={styles.sheetActions}>
              <Pressable style={[styles.sheetCancel, { borderColor: colors.border }]} onPress={closeIosPicker} accessibilityRole="button">
              ...
              <Pressable style={[styles.sheetDone, { backgroundColor: colors.primary }]} onPress={() => applyIosPicker()} accessibilityRole="button">
...
const styles = StyleSheet.create({
  ...
  previewCard: { flexDirection: 'row', alignItems: 'center', gap: 14, padding: 16, borderRadius: 18, borderWidth: 1 },
  ...
  infoBanner: { flexDirection: 'row', alignItems: 'flex-start', gap: 9, padding: 12, borderRadius: 14, borderWidth: 1 },
  ...
  quickBtn: { flex: 1, flexDirection: 'row', alignItems: 'center', justifyContent: 'center', gap: 8, paddingVertical: 12, borderRadius: 14, borderWidth: 1 },
  ...
  timeChip: { flexDirection: 'row', alignItems: 'center', gap: 6, paddingHorizontal: 12, paddingVertical: 7, borderRadius: 10, borderWidth: 1 },
  ...
  summaryRow: { flexDirection: 'row', alignItems: 'center', gap: 9, padding: 13, borderRadius: 14, borderWidth: 1 },
  summaryTxt: { flex: 1, fontSize: 12.5, fontWeight: '600', lineHeight: 17 },
  footer: { position: 'absolute', left: 0, right: 0, bottom: 0, paddingHorizontal: 16, paddingTop: 12, borderTopWidth: StyleSheet.hairlineWidth, gap: 10 },
  errorBox: { borderRadius: 12, borderWidth: 1, padding: 11 },
  errorText: { fontSize: 12.5, fontWeight: '700', lineHeight: 17 },
  saveBtn: { flexDirection: 'row', alignItems: 'center', justifyContent: 'center', gap: 8, borderRadius: 16, paddingVertical: 16, shadowColor: '#000', shadowOffset: { width: 0, height: 4 }, shadowOpacity: 0.12, shadowRadius: 10, elevation: 4 },
  saveText: { color: '#FFFFFF', fontWeight: '900', fontSize: 16 },
  modalBackdrop: { flex: 1, backgroundColor: 'rgba(0,0,0,0.45)' },
  sheet: { borderTopLeftRadius: 22, borderTopRightRadius: 22, paddingHorizontal: 18, paddingTop: 10, paddingBottom: 24, borderTopWidth: 1 },
  ...
  sheetCancel: { flex: 1, flexDirection: 'row', alignItems: 'center', justifyContent: 'center', gap: 6, paddingVertical: 14, borderRadius: 14, borderWidth: 1 },
